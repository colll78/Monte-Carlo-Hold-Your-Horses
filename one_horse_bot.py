...
def displayState(state, playerNames, selectedSquare, currentMove=None, moveProgress=0):
    global boardWidth, boardHeight, win
...
maxLookAhead = 5  # Maximum search depth
reachable = None
apple_loc = None
mating_squares = None
knight_distance = {}
...
def move_score(move, state):
    (xStart, yStart, xEnd, yEnd) = move
    move_heuristic = 0
    mobility_priority = 4
    distance_priority = 4
    capture_priority = 18
    secure_capture_priority = 25
    safe_move_priority = 14
    threat_priority = 3

    curr_knight_dist = knight_distance.get((xStart, yStart, -state.playerToMove))
    after_move_dist = knight_distance.get((xEnd, yEnd, -state.playerToMove))
    distance_dif = after_move_dist - curr_knight_dist

    # apple distance score
    move_heuristic += distance_priority if distance_dif < 0 else -distance_priority if distance_dif > 0 else 0
    #print("Move apple distance dif: ", distance_priority if distance_dif < 0 else -distance_priority if distance_dif > 0 else 0 )
    old_reachable = reachable[(xStart, yStart)]
    new_reachable = reachable[(xEnd, yEnd)]

    # mobility score
    mobility_score = len(new_reachable) - len(old_reachable)
    move_heuristic += mobility_score if -mobility_priority <= mobility_score <= mobility_priority else -mobility_priority if mobility_score < -mobility_priority else mobility_priority
    #print("Mobility: ", mobility_score if -mobility_priority <= mobility_score <= mobility_priority else -mobility_priority if mobility_score < -mobility_priority else mobility_priority)

    # secure capture score
    atk_def = attackers_defenders(state, xEnd, yEnd)
    move_heuristic += secure_capture_priority if atk_def >= 0 and state.board[xEnd, yEnd] == -state.playerToMove else 0
    #print("secure capture? 25 points", atk_def >= 0 and state.board[xEnd, yEnd] == -state.playerToMove)

    # capture score
    move_heuristic += capture_priority if atk_def < 0 and state.board[xEnd, yEnd] == -state.playerToMove else 0
    #print("capture: ", capture_priority if atk_def < 0 and state.board[xEnd, yEnd] == -state.playerToMove else 0)

    # safe moves
    move_heuristic += safe_move_priority if atk_def >= 0 else 0
    #print("safe move points; ", safe_move_priority if atk_def >= 0 else 0)
                ...
                for (dx, dy) in direction:  # Check all potential move vectors
                    ...
                    if xEnd >= 0 and xEnd < boardWidth and yEnd >= 0 and yEnd < boardHeight and not (state.board[xEnd, yEnd] in [state.playerToMove, 2 * state.playerToMove]):
                        ...
                        if state.board[xEnd, yEnd] == state.playerToMove * -2:
                            return [((xStart, yStart, xEnd, yEnd), 1000)]
                        ...
                        # secure capture score
                        atk_def = attackers_defenders(state, xEnd, yEnd)
                        move_heuristic += secure_capture_priority if atk_def >= 0 and state.board[
                            xEnd, yEnd] == -state.playerToMove else 0
                        # print("secure capture? 25 points", atk_def >= 0 and state.board[xEnd, yEnd] == -state.playerToMove)

                        # capture score
                        move_heuristic += capture_priority if atk_def < 0 and state.board[
                            xEnd, yEnd] == -state.playerToMove else 0
                        # print("capture: ", capture_priority if atk_def < 0 and state.board[xEnd, yEnd] == -state.playerToMove else 0)
                        ...
                        moves.append(((xStart, yStart, xEnd, yEnd), move_heuristic))
                        #moves.append((xStart, yStart, xEnd, yEnd))
    moves.sort(reverse=True, key=lambda x: x[1])
    #filtered_moves = [x[0] for x in moves]
    #print("POST LIST :", filtered_moves[:5])
    # print(moves)
    #print(moves)
    # filtered_moves = [x for x in moves if attackers_defenders(state, x[2], x[3]) >= 0]
    end = timer()
    filter_time += end - start
    #print("filter_possible_moves: ", (end-start))
    #print(filtered_moves)
    return moves

def filter_possible_moves2(state):
...
def getScore(state):
    ...
    for i in range(0, len(p1_horses[0])):
        ...
        # displayState(state, playerNames=playerNames, selectedSquare=(p1_horses[0][i], p1_horses[1][i]))
        # print("Horse x,y: %s %s" % (p1_horses[0][i], p1_horses[1][i]))
        # print(appleDistance)
        # input()
    ...
    for i in range(0, len(p2_horses[0])):
        ...
        # displayState(state, playerNames=playerNames, selectedSquare=(p2_horses[0][i], p2_horses[1][i]))
        # print("Horse x,y: %s %s" % (p2_horses[0][i], p2_horses[1][i]))
        # print(appleDistance)
        # input()
        neighbors = 0
        score -= pieceValue - appleDistance
...
def lookAhead(state, depthRemaining, alpha, beta):
    ...
    # print("Move opts: ", len(getMoveOptions(state)))
    # print("filtered ", len(filter_possible_moves(state)))
    # input("h")
    if state.playerToMove == 1:
        bestScore = -9e9
        for move in filter_possible_moves(state):
            projectedState = makeMove(state, move[0])  # Try out every possible move...
            score = lookAhead(projectedState, depthRemaining - 1, alpha, beta)  # ... and score the resulting state
            bestScore = max(score, bestScore)
            # if score > alpha:
            #     displayState(projectedState, playerNames=playerNames, selectedSquare=(move[0], move[1]))
            #     print("Score: %s" % score)
            ...
            #     else:
            #         print("is minimizing")
            #     print("Horse x,y: %s %s" % (move[0], move[1]))
            #     input()
        ...
        for move in filter_possible_moves(state):
            projectedState = makeMove(state, move[0])  # Try out every possible move...
            score = lookAhead(projectedState, depthRemaining - 1, alpha, beta)  # ... and sc
...
def initPlayer(_startState, _timeLimit, _victoryPoints, _moveLimit, _assignedPlayer):
    global startState, timeLimit, victoryPoints, moveLimit, assignedPlayer, boardWidth, boardHeight, apple_loc, mating_squares, knight_distance, reachable, win, playerNames
    ...
    if assignedPlayer == 1:
        playerNames = ["one_horse_bot", 'opponent']
    else:
        playerNames = ['opponent', 'one_horse_bot']
...
def getMove(state):
    ...
    favoredMove = moveList[0][0]  # Just choose first move from the list for now, in case we run out of time
    favoredMoveScore = -9e9 * state.playerToMove  # Use this variable to remember the score for the favored move

    # Iterative deepening loop
    for lookAheadDepth in range(2, maxLookAhead + 1): #4):
        ...
        for move in moveList:
            projectedState = makeMove(state, move[0])
            ...
            # displayState(projectedState, playerNames=playerNames, selectedSquare=(move[0][0], move[0][1]))
            # print("Score: %s" % score)
            # input()
            # print("player to move: -1")
            # if assignedPlayer > 0:
            ...
            # else:
            #     print("is minimizing")
            # print("MOVE SCORE: ", move_score(move[0], state))
            # print("Horse x,y: %s %s" % (move[0], move[1]))
            if timeOut():
                break

            if (state.playerToMove == 1 and score > currBestScore) or (state.playerToMove == -1 and score < currBestScore):
                currBestMove, currBestScore = move[0], score  # Found new best move during this iteration
        ...
        if not timeOut():  # Pick the move from the last lookahead depth as new favorite, unless the lookahead was incomplete
            ...
            duration = datetime.now() - startTime
            print('one_horse_bot: Depth %d finished at %.4f s, favored move (%d,%d)->(%d,%d), score = %.2f'
