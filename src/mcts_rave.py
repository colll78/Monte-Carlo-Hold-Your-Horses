...
class HorseHoldRaveSearchNode():
    ...
    def best_child(self, c_param=exploration_constant):
        ...
        for c in self.children:
            ...
            if decaying_w:
                weight = (1 - decaying_w) * (c.node_score / c.num_visits) + decaying_w * (c.move_win_count / c.num_simulations_containing_move) + c_param * np.sqrt((2 * np.log(self.num_visits) / c.num_visits))
            else:
                weight = c.node_score / c.num_visits + c_param * np.sqrt((2 * np.log(self.num_visits) / c.num_visits))
